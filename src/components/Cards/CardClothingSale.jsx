...
export default function CardClothingSale({
  product,
  functionSelectProduct,
  functionToDeleteProduct
}) {
  ...
  return (
    <div onClick={handlerClick} className="flex flex-col justify-center w-full min-h-36 max-h-3min-h-36 shadow-lg rounded-lg border-[#cecece] border-2 px-2 py-2">
      <div className="flex flex-col justify-between min-w-[100%] max-w-[100%]">
        <div className="flex flex-col min-h-[5rem]">
          <div className="flex w-full">
            <span className="text-2xl font-bold capitalize">{product.productName}</span>
            <MdDelete onClick={handlerDelete} className="text-4xl text-[#ff1414] ml-auto cursor-pointer" />
          </div>
